/* CUDA reduction exercise (summation in this case). The initial code
   ...
   reduction at the top level (end of the kernel). (This will only
   work for single precision floats - why?) More specifically, the
...
To compile for K40/80 GPUs (mosaic, copper)

  nvcc -arch=sm_35 -O2 reduction.cu -o reduction

To compile for angel:

  nvcc -arch=sm_50 -O2 reduction.cu -o reduction
